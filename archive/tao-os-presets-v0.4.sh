...
# Usage: ./tao-os-presets-v0.4.sh --apply-temp   or   --undo
...
STATE_FILE="$HOME/TAO-OS/preset_state_backup_v0.4.txt"

if [[ "$ACTION" == "--apply-temp" ]]; then
    echo "Applying temporary mining presets (v0.4)..."
    ...
    echo ""
    echo "All v0.4 presets applied (temporary – reboot or --undo to revert)."
    echo "Run ./benchmark-v0.8-vanilla.sh to measure lift."

elif [[ "$ACTION" == "--undo" ]]; then
    if [[ -f "$STATE_FILE" ]]; then
        echo "Reverting v0.4 presets..."
