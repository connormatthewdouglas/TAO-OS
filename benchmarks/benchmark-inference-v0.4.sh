#!/usr/bin/env bash
# TAO-OS benchmark-inference-v0.4.sh
# Cold-start latency benchmark: measures GPU wakeup + model load penalty.
# v0.4: Extended data collection (hardware fingerprint + stability metrics)
#
...
#   NEW: Also captures hardware fingerprint and stability metrics.
#
# Usage: ./benchmark-inference-v0.4.sh [preset-script] [model]
#   preset-script : default ./tao-os-presets-v0.7.sh
...
# ── Header ────────────────────────────────────────────────────────────────────
log "TAO-OS Inference Benchmark v0.4 — Cold-Start Latency (with Extended Data)"
log "Model:      $MODEL"
...
# ── NEW: Collect hardware fingerprint once at session start ─────────────────────
log ""
log "Capturing extended hardware data..."
inject_hardware_fingerprint "$LOG_FILE"
log "  ✓ Hardware fingerprint captured"

# ── PASS 1: Baseline ──────────────────────────────────────────────────────────
log ""
log "PASS 1 — BASELINE (no presets, GPU idles to default freq)"

# Capture baseline stability state
log "  Capturing pre-baseline stability metrics..."
inject_stability_metrics "$LOG_FILE" "baseline" "$STABILITY_BASELINE_FILE"

run_pass "BASELINE"
BASELINE="$PASS_RESULT"
BASELINE_TOKS=$(echo "$BASELINE_TOKS_PER_SEC" | cut -d' ' -f1 2>/dev/null || echo "N/A")

# Capture post-baseline stability state
inject_stability_metrics "$LOG_FILE" "baseline" "$STABILITY_BASELINE_FILE"
...
rm -f "$STABILITY_BASELINE_FILE"

# ── PASS 2: Tuned ─────────────────────────────────────────────────────────────
log ""
log "PASS 2 — TUNED (GPU pinned to min 2000 MHz)"

# Capture pre-tuned stability state
log "  Capturing pre-tuned stability metrics..."
inject_stability_metrics "$LOG_FILE" "tuned" "$STABILITY_BASELINE_FILE"

run_pass "TUNED"
TUNED="$PASS_RESULT"
TUNED_TOKS=$(echo "$TUNED_TOKS_PER_SEC" | cut -d' ' -f1 2>/dev/null || echo "N/A")

# Capture post-tuned stability state
inject_stability_metrics "$LOG_FILE" "tuned" "$STABILITY_BASELINE_FILE"
...
if (( $(echo "$BASELINE > 0" | bc -l) )); then
    ...
    DELTA_MS=$(echo "scale=1; $TUNED - $BASELINE" | bc -l)
    DELTA_LABEL="negative = improvement (less latency)"
else
    DELTA="N/A"
    DELTA_MS="N/A"
    DELTA_LABEL=""
...
log "  Delta:            ${DELTA_MS}ms (${DELTA}%)  ($DELTA_LABEL)"
log "  Baseline tokens:  ${BASELINE_TOKS} tok/s"
log "  Tuned tokens:     ${TUNED_TOKS} tok/s"
log "========================================"
log "Log: $LOG_FILE"
log "Complete: $(date)"
